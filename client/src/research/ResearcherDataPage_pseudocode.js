...
class ResearcherDataPage extends Component {
  ...
  componentDidMount() {
    fetch(get locations)
      .then(
        for (location,description) in results:
          analyses.append({
            description,
            location,
            filter(row) {
              return _.every([
                Filters.containsPath(location, row),
          ...
          this.setState.analysis = _.first(analyses)
          })
  }

  onAnalysisChanged(e, index, targetAnalysisKey) {
    const analysisTuple = _.find(_.entries(this.state.analyses), pair => pair[0] === targetAnalysisKey);
  ...
  renderSelect(currentKey) {
    return (
      ...
      >
        {_.entries(this.state.analyses).map((analysisTuple) => {
          const [key, analysis] = analysisTuple;
