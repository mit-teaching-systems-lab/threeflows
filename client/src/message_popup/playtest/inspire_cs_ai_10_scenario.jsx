...
`, force: true, writeNoPrompt: true});
  ...
  slides.push({ type: 'Enact', text:`After the initial check-in, the conversation turns to the student's roles and contributions to the project. Imani: “I like to do the writing on my own.”
...
`, force: true, writeNoPrompt: true});
  // Please add selection buttons "Continue Listening" or "Intervene" and allow them to type a response. 
  slides.push({ type: 'Enact', text:`Imani: “And Hakeem is best at coding. We won’t have time to get it all done if he doesn’t do it. Besides, it’s too hard to share accounts so that more than one person can code.”
...
`, force: true, writeNoPrompt: true});
  slides.push({ type: 'Enact', text:`During lunch Julio comes to your classroom. “Can I make my own app? I really want to make an app about identifying different types of spiders. I think that would be really cool. And I’m not really sure what my group is doing.” 

How do you respond to Julio? 
`, force: true, writeNoPrompt: true});
  slides.push({ type: 'Enact', text:`The following Tuesday, you notice that Imani is pair programming with Hakeem. You see that Jeremy and Julio are creating the slides and they are about half way done. 

You walk over to their table to check in. What do you say?
`, force: true, writeNoPrompt: true});
  slides.push({ type: 'Enact', text:`After the group check-in the students bring up their grades. 
...
`, force: true, writeNoPrompt: true});
  ...
  slides.push({ type: 'Reflect', text:`What do you think the experience of each student in the group was? Were there dynamics related to gender, race, or ability?`, force: true, writeNoPrompt: true});
