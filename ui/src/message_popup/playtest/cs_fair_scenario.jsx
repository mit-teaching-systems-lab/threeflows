...
const projects = {
  A: { label: 'A', el: 
    <div>
      ...
      <p>
        2A) The computing innovation that is represented by my computational artifact is that of virtual reality. Virtual
        reality is using technology to create a reality outside of the real world by tricking fooling your brain to think
        you are in a different environment. The artifact attached with this starts by going through the basic functions
        that allow vr to work. With the headset and a cpu working together to seamlessly stream this information
        back and forth. The bottom part goes more into the stats associated with vr, and that this is a growing
        industry for young people, and seesm to be on the move to grow even more.
      </p>
      <p>
        2B) I used a website called piktochart to create my artifact. So what I did is I split up the infographic area into
        separate blocks to contain the graphics on how vr works and the other block on stats associated with it. I
        then chose pictures off of the internet that fit the background with a vr headset and computer tower so that I
        could simulate the stream of data in between and how the system works as a whole and show that it is a
        cycle. I then added all of my text to explain this along with arrows that were in the graphics area to show the
        cycle that the data takes. I found some graphs that showed huge popularity and projected popularity and
        added those in with an explanation to show how big vr is becoming.
      </p>
      <p>
        2C) A beneficial effect that vr has on society is the ability to run simulations using this system. For the things in
        career paths with dangerous situations it is much better to be able to simulate this using a created
        environment where no one will get hurt but still allows for people in training to learn valuable skills.(3)
        Another great effect is that it can revolutionize the entertainment industry with the ability to go on film sets or
        into a video game, or even see a fashion show without being there and experiencing it as if you were.(2)
        This really makes you feel like you are there and get all new experiences out of it you might have never
        been able to get before. There are harmful effects as well. One of the harmful effects is that with this being
        such an immersive technology many people tend to forget about their real life and only focus on the virtual
        one.(3) This can lead many to be much more antisocial and distract them from what they should be doing.
        This is why society as a whole still needs to do some thinking on the subject of vr as it has huge possible
        social consequences that could affect many later on down the road.
      </p>
      <p>
        2D) The kind of data that vr would work primarily with are visual representation with RGB values, user input such
        as direction of view or 3d location of headset, and the sound files that go with it all. The headpiece tracks
        the users relative position to previously and sends this to cpu in the form of coordinates in this virtual world.
        The cpu takes that data and moves the user in its virtual reality that it is keeping track of. It decides how the
        screen should change to to keep up this new environment as it is tracking of the movement of the user and
        sends it to the user so that they receive believable imagery and sound that they believe they are in this new
        environment. If there is any lag or lack of bandwidth in between the headset and cpu then the user
        immediately tells out that it is not real and this the new world falls apart. That is why this data stream must
        be seamless in the communication between the headset and the cpu for the user to receive a great time.
        This creates issues that you need to be able to transfer huge gobs of data in this connection in as little time
        as possible. This creates issues of how much ram the cpu has and as of now most vr systems require about
        a 8 gb of ram.(1)
      </p>
      <p>
        2E) 1 http://iq.intel.com/howvirtualrealityworks/, Kevin Ohannessian, "How Virtual Reality Works," source:
        intel , date viewed: 1/26/16
        2 http://www.forbes.com/sites/rachelarthur/2016/01/20/virtualrealityandrobotsheadforstockholminlatestfashionweektechnologyintegration/#2715e4857a0b2a40148f6600,
        Rachel Arthur, "Virtual Reality
        And Robots Head For Stockholm In Latest Fashion Week Technology Integration," source: Forbes, date
        viewed: 1/25/16, posted: 1/20/16
        3 http://reporter.rit.edu/views/dangersgivingvirtualreality, Tim Henry, "The Dangers of Giving in to
        Virtual Reality," source: Reporter, date viewed: 1/25/16, posted: 10/2/14
      </p>
    </div>
  },
  E: { label: 'E', el:
    <div>
      <iframe width="100%" height="315" src="https://www.youtube.com/embed/QZFLT_awBqI" frameBorder="0"></iframe>
      <p>
        Name: IPhone 6s
  ...
  C: { label: 'C', el:
    <div>
      <img alt="Loading artifact..." src="https://tsl-public.s3.amazonaws.com/threeflows/csfair-c.png" width="100%" height="448" />
      <p>
  ...
  D: { label: 'D', el:
    <div>
      <iframe width="100%" height="315" src="https://www.youtube.com/embed/06ntwM2z0UI" frameBorder="0"></iframe>
      <p>
...
`});
  ...
  slides.push({ type: 'Try it!', el: project.el});
...
`, textResponse: true});
  ...
  /*
  See:
  - Sample responses and commentary: http://apcentral.collegeboard.com/apc/public/exam/exam_information/231726.html
  - Scoring guidelines: https://secure-media.collegeboard.org/digitalServices/pdf/ap/explore-sample-tasks-sg.pdf
  - Commentary: http://www.doralacademyprep.org/ourpages/auto/2017/2/22/60685409/create-sample-tasks-commentary.pdf
  - Q&A: https://secure-media.collegeboard.org/digitalServices/pdf/ap/explore-student-performance_q-and-a.pdf
  */
...
https://docs.google.com/document/d/1uzylD9CNW0M6EnS3B4X3YcU9ArSNcLsO80m97lWLQ0s/edit#
https://docs.google.com/document/d/1_Q9D1utxDup_4yc1frFCcnEboy_THvsPeIkARZ4by0s/edit#
*/
