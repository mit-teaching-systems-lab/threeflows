...
This has a few changes:
 - expose methods
 - replace `end` variable with `endValue`
*/
/**
 * AuO.js
 *
...
export function AuO(/* SAVE_URL = null, SAVE_CALLBACK = null, LOCAL_SAVE_CALLBACK */) {
  ...
  const run = function () {
     ...
     */
  const suspendInstance = this.suspendInstance = function () {
    if (!state.running) {
      return;
    }
     ...
     */
  const stateReset = this.stateReset = function () {
  ...
  const beginAudioPlayback = function (/* start = 0, end = Infinity */) {
    ...
    var endVal = getArgument(arguments, 1, Infinity);
    if (Infinity === endVal) {
      endVal = state.elapsedTime;
    }
  ...
  const beginAudioSave = function (onready /* , start = 0, end = Infinity */) {
    ...
    var endVal = getArgument(arguments, 2, Infinity);
    if (Infinity === endVal) {
      endVal = state.elapsedTime;
    }
    ...
    if ("wav" === format) {
      ...
      const endSample = Math.min(saveBuffer.length, Math.round(endVal * sampleRate));
  ...
  const onRecordClicked = this.onRecordClicked = function (event) {
    container.element().focus();
  ...
  const onStopClicked = this.onStopClicked = function (event) {
    container.element().focus();
  ...
  const onSaveClicked = this.onSaveClicked = function(event) {
    container.element().focus();
